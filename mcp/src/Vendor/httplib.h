 ...
 */
    ...
    public:
        ...
        Server& set_header_writer(const std::function<ssize_t(Stream&, Headers&)>& writer);
    ...
    public:
        ...
        void set_header_writer(const std::function<ssize_t(Stream&, Headers&)>& writer);
    ...
    private:
        ...
        Result
        send_with_content_provider(const std::string& method, const std::string& path,
...
#define USE_IF2IP
        ...
        bool read_content(Stream& strm, T& x, size_t payload_max_length, int& status,
                          Progress progress, ContentReceiverWithProgress receiver, bool decompress)
        {
                                               ...
                                               size_t content_length, std::string& data)
        {
            process_multipart_ranges_data(
                ...
                { data += token; }, [&](const std::string& token) { data += token; },
                                                ...
                                                size_t content_length, const T& is_shutting_down)
        {
            return process_multipart_ranges_data(
                req, boundary, content_type, content_length, [&](const std::string& token)
                { strm.write(token); }, [&](const std::string& token) { strm.write(token); },
                [&](size_t offset, size_t length)
                {
        ...
        inline bool parse_www_authenticate(const Response& res,
                                           std::map<std::string, std::string>& auth, bool is_proxy)
        {
    ...
    inline void
    ClientImpl::set_header_writer(const std::function<ssize_t(Stream&, Headers&)>& writer)
    {
        header_writer_ = writer;
    }
 ...
 */
    ...
    namespace detail
    {
        ...
        template <typename T>
        inline bool
        process_server_socket_ssl(const std::atomic<socket_t>& svr_sock, SSL* ssl, socket_t sock,
  ...
  */
    ...
    inline void Client::set_header_writer(const std::function<ssize_t(Stream&, Headers&)>& writer)
    {
