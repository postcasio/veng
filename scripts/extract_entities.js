...
const mapFile = (0, fs_1.readFileSync)(process.argv[2], 'utf8');
/*
entity looks like:
...
if (entities) {
    for (const entityMatch of entities) {
        ...
        const entity = JSON.parse(entityMatch[0]
            .replace(/"(\w+)"\s+"([^"]*)"/g, '"$1": "$2",')
            .replace(/"(\w+)"\s+"([^"]*)"\n/g, '"$1": "$2"\n')
...
(0, fs_1.writeFileSync)(process.argv[2].replace(/\.map$/, '.json'), JSON.stringify(jsonEntities, null, 2));
