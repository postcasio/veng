...
| 02 | [Reflection-driven inspector](02-reflection-inspector.md) | Wire `TypeRegistry`/`FieldDescriptor` into the inspector panel. Widget per `FieldClass`. Selecting an ECS entity populates the inspector with its components. | proposed |
| 03 | [Cook-on-demand plumbing](03-cook-on-demand.md) | Link `libveng_cook` into the editor host. Off-thread `CookRequest` via `TaskSystem`, in-memory scratch archive, hot-reload via `AssetManager` on the main thread. | proposed |
