...
| # | Plan | Summary | Status |
|---|---|---|---|
| 01 | [Mesh runtime material model](01-mesh-material-model.md) | Replace `SubMesh`'s `AssetId Material` with a `u32 MaterialIndex` into a resident `vector<AssetHandle<Material>>` on `Mesh`; migrate `MeshLoader` to eager-resolve cooked submesh ids → that list (reversing planset-5's lazy rule); update the sample to a per-submesh material draw. Cooked on-disk format unchanged. | proposed |
