...
- Construct one `SceneRenderer` and render a sequence of **K > `MaxFramesInFlight`** frames
  where each frame's rendered content is **distinguishable by frame index** — e.g. animate
  the camera or a clear value so frame i has a known signature at a sampled texel.
- Each frame: `Execute` the renderer, then **capture that frame's output** the way a
  consumer would (transition + copy it into a per-frame readback buffer through the normal
  barrier path), **without** waiting idle between frames — let the engine's
  `MaxFramesInFlight` pipelining keep multiple frames live.
...
output, no extra barrier) so its result is the honest measurement of the current state.
...
hazards regardless of whether they corrupted *this* run. The engine **already enables it**
(`eSynchronizationValidation` in `Context.cpp`'s validation-feature list, behind
`VE_ENABLE_VALIDATION_LAYERS`), and sync hazards flow through the debug messenger as
validation errors:
...
- Wire this as an extension of the existing validation gate (`cmake/ValidationGate.cmake` +
  ...
  hazard. Because the smoke renders one frame, this overlap test is what first makes the
  sync-validation gate meaningful for cross-frame hazards at all.
