...
| # | Plan | Summary | Status |
|---|---|---|---|
| 00 | [Unify the reflection trait](00-unify-reflect-trait.md) | Collapse `ReflectLeaf<T>` into `VengReflect<T>`: uniform member set, `VE_LEAF` macro, direct-read `TypeIdOf`/`FieldClassOf`, delete `IsReflectLeaf` + `EnsureLeaf`, `Class` as member. Migrate engine builtins + cooker + tests. Behaviour-preserving. | proposed |
...
| 02 | [Rename `assetformat` → `assetpack`](02-rename-assetformat.md) | Mechanical rename of the shared archive/cooked-blob library: directory, CMake target + `veng::` alias, and comments. No header path, API, or `.vengpack` format change. | proposed |
