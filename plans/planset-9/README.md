...
| 03 | [Context-owned in-memory cache](03-in-memory-cache.md) | B | A `vk::PipelineCache` in `Context::Native` + a `GetVkPipelineCache` accessor, threaded into both pipeline factories. No disk; pixels unchanged. | proposed |
...
| 05 | [Format v2 + cooker writes hashes](05-format-and-write.md) | C | `ContentHash` per TOC entry + a header digest over the serialized TOC bytes; `ArchiveFormatVersion` → 2; `assetformat` round-trips (no hash dep); cooker computes xxh3-128 + writes; loader reads-but-ignores; re-cook packs. | proposed |
