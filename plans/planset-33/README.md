...
| 01 | Texture mip chain | Offline mip generation in the texture cooker (sRGB-/linear-correct), the multi-mip blob layout, and a multi-region upload (a new multi-region `CopyBufferToImage`) — replacing the single-mip-only restriction. Still uncompressed RGBA8; lays the infrastructure Plan 02 needs. | done |
