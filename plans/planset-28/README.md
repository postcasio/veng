...
| 02 | [Selectable Dual Kawase kernel](02-dual-kawase-kernel.md) | `SceneRendererSettings` gains a `BloomKernel { Cod, Kawase }` enum + `Kernel` field (default `Cod`); the down/up compute shaders gain the Kawase tap variant, selected at `Configure`. The `Cod` golden is unchanged; a `gpu` test asserts `Kawase` blooms a bright region too. Depends on 01. | ready |
