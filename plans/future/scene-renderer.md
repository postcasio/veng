...
- **Depth/stencil-masked deferred lighting.** Image-based lighting + an environment skybox are
  delivered: an `Environment` asset (an equirectangular HDR panorama) drives radiance/irradiance/
  prefiltered-specular cubemaps + a BRDF LUT generated by compute (`EnvironmentIbl`, off bindless),
  the lighting pass evaluates split-sum IBL through a dedicated set, and a separate `SkyboxScenePass`
  composites the radiance cube over the cleared-depth background. Both the lighting and skybox passes
  are fullscreen and run on **every** pixel, masking foreground/background with an in-shader depth
  sample + `discard`. The refinement: mark sky vs. geometry pixels once (a stencil write in the
  g-buffer pass, or a depth-bounds/`EQUAL` test) so the lighting pass runs **only** where geometry
  exists and the skybox runs **only** on the background — no wasted lighting work on sky pixels and no
  branch divergence at silhouettes. It also gives a hardware-depth-tested home for a richer procedural
  sky model (atmospheric scattering, sun disc) replacing the cube sample in the skybox pass.
