...
(area 4), `ImGuiCompositePass` (area 11), the UI toolkit (area 12), and the named
still-future increments of the areas done in part (areas 1, 2, 7, 8, 9, 10).
...
### 12. UI toolkit — `Veng::UI`

17 files call `ImGui::` directly today, each choosing its own widget, flags, and
spelling. An engine-tier **`Veng::UI`** vocabulary (in `libveng`, so games *and*
the editor author against it) fronts ImGui with a thin, opinionated, overload-
driven surface: one `Drag` overloaded on `f32`/`vec2`/`vec3`/`vec4`/`i32`,
designated-initializer options structs instead of ImGui flags, and RAII scopes for
the `Begin`/`End` and `PushID`/`PopID` pairs. Configurability is deliberately
reduced for UI consistency. Complex stateful widgets (a file browser) are editor-
tier classes whose `Draw()` returns an event. **Wrapper-only:** ImGui stays a
PUBLIC dependency this round — driving it fully private (the Native-idiom end-state)
is a possible later planset that `Veng::UI` unblocks. **Design overview:**
...
1. **Editor — scene editor (area 6, sub-area D)** is the **next editor planset**;
   ...
   sub-areas B/C).
2. **Event & input (area 4)**, **`ImGuiCompositePass` (area 11)**, the **UI
   toolkit (area 12)**, and the named still-future increments of the areas done in
   part (1, 2, 7, 8, 9, 10) are each independent and off the critical path — slot in
   whenever wanted.
...
area 12 (UI toolkit — `Veng::UI`), and the named still-future increments of the
