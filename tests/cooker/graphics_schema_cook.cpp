...
TEST_CASE("graphics schema cook: a valid VisibleWhen dependency cooks and round-trips")
{
    json schema = SampleSchema();
    // fov becomes visible only while shadows is 'high' — a dependency on a setting in another
    // category, which the second validation pass resolves.
    schema["Categories"][1]["Settings"][0]["VisibleWhen"] = "shadows";
    schema["Categories"][1]["Settings"][0]["VisibleWhenOption"] = "high";
    const path packJson = WriteSchemaPack("gfxschema_visiblewhen", schema);
    ...
    REQUIRE(ReadFields(record, &data, registry.Info(TypeIdOf<GraphicsSchemaData>()), registry)
                .has_value());
    CHECK(data.Categories[1].Settings[0].VisibleWhen == "shadows");
    CHECK(data.Categories[1].Settings[0].VisibleWhenOption == "high");
}

TEST_CASE("graphics schema cook: a VisibleWhen naming a missing setting is a located error")
{
    json schema = SampleSchema();
    schema["Categories"][1]["Settings"][0]["VisibleWhen"] = "clouds";
    schema["Categories"][1]["Settings"][0]["VisibleWhenOption"] = "on";
    const string error = CookExpectingError("gfxschema_visiblewhen_missing", schema);
    REQUIRE_FALSE(error.empty());
    CHECK(error.find("clouds") != string::npos);
}

TEST_CASE(
    "graphics schema cook: a VisibleWhenOption the target does not declare is a located error")
{
    json schema = SampleSchema();
    schema["Categories"][1]["Settings"][0]["VisibleWhen"] = "shadows";
    schema["Categories"][1]["Settings"][0]["VisibleWhenOption"] = "ultra"; // shadows: off/high
    const string error = CookExpectingError("gfxschema_visiblewhen_badopt", schema);
    REQUIRE_FALSE(error.empty());
    CHECK(error.find("ultra") != string::npos);
}
