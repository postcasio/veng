...
namespace Veng::UI
{
    namespace
    {
        // FontAwesome icon macros are u8 string literals; view their bytes as a UTF-8 char string
        // for the icon-button glyphs (the engine font merges the FontAwesome range).
        const char* Glyph(const char8_t* glyph)
        {
            return reinterpret_cast<const char*>(glyph);
        }

        const char* const AddGlyph = Glyph(ICON_FA_PLUS);
        const char* const RemoveGlyph = Glyph(ICON_FA_XMARK);

        // A named combo over the enum's {name, value} table when its TypeInfo carries one, or an
        // editable integer view over the backing bytes (width from its TypeInfo) when it does not.
        // A backing value matching no enumerator shows a synthesized "(unknown N)" label so it is
        // never silently wrong; picking a named entry repairs it. Returns true when the value changed.
        bool DrawEnum(void* fieldPtr, const FieldDescriptor& field, string_view label,
                      const InspectorHooks& hooks)
        {
            const TypeInfo& info = hooks.Registry->Info(field.Type);
            ...
            {
                // The synthesized entry is not a real enumerator, so a pick lands only on a named
                // one; writing back narrows the chosen value into the low Size bytes.
    ...
    // Recurses each non-hidden field of a struct/active-alternative as an indented row; shared by
    // the Struct, Variant, and Array-element cases. Routes through the shared DrawFields walk so
    // nested fields get the same Category grouping the top-level walk does. Returns true when any
    // nested field changed.
    static bool DrawStructFields(void* base, const TypeInfo& info, const InspectorHooks& hooks)
    {
        UI::Indent();
        const bool changed = DrawFields(base, info.Fields, hooks);
        UI::Unindent();
        return changed;
    }
    ...
    bool DrawFields(void* base, std::span<const FieldDescriptor> fields,
                    const InspectorHooks& outerHooks)
    {
        VE_ASSERT(outerHooks.Registry != nullptr, "InspectorHooks::Registry is required");

        // Re-seed the owner base to the struct this walk iterates, so a field's predicate reads its
        // immediate owner — a nested struct's recursion (through DrawStructFields → DrawFields)
        // seeds the nested base, an array element seeds the element base.
        InspectorHooks hooks = outerHooks;
        hooks.OwnerBase = base;
        ...
        for (const FieldDescriptor& field : fields)
        {
            ...
            {
                changed |= DrawGatedField(base, field, hooks);
                continue;
            }
        ...
        // Each category renders under a full-width collapsing header, its fields in declared order.
        // The header is a no-op extra row when no field carries a Category.
    ...
    namespace
    {
        ...
        // property-table value column — the caller has already drawn the label and pushed the field
        // id. Shared by DrawFieldWidget (every non-composite field) and DrawArray (a leaf element).
                         ...
                         const InspectorHooks& hooks)
        {
            ...
            if (UI::Combo(label, index, items))
            {
                // Carry the material across the switch: a default-constructed alternative has a null
                // material, and the renderer skips a submesh that has none — so without this a type
                // change would rebuild a mesh that renders nothing. The handle is copied out before
                // SetActive destructs the outgoing alternative; shared scalar parameters
                // intentionally reset to the new shape's defaults.
            ...
            if (activePtr != nullptr)
            {
                ...
                changed |= DrawStructFields(activePtr, activeInfo, hooks);
            }
            return changed;
        }

        // An add/remove list over a FieldClass::Array field. The label row carries an Add button.
        // A leaf element draws as a single value row with a trailing Remove button. A struct/variant
        // element draws its fields either flat (a per-element row bearing a Remove button) or, when
        // `collapsible`, under a foldable header whose row carries the Remove button. Reorder is not
        // offered — the configuration list is order-independent.
        //
        // The element fold state is ImGui-owned, keyed on the positional per-element id
        // ({label}elem{i}); the elements have no stable identity to key on, so removing element [i]
        // slides every later element's fold state down one. This is harmless precisely because the
        ...
        bool DrawArray(void* fieldPtr, const FieldDescriptor& field, string_view label,
                       const InspectorHooks& hooks, bool collapsible, bool defaultOpen)
        {
            const TypeRegistry& registry = *hooks.Registry;
            ...
            if (UI::IconButton(fmt::format("{}##add{}", AddGlyph, label)))
            {
            ...
            // struct/variant element, which recurses its fields. Loop-invariant in the element type,
            ...
            for (usize i = 0; i < count; ++i)
            {
                ...
                if (elementIsLeaf)
                {
                    // One row: the element's value widget fills the column, the Remove button trails
                    // it. The synthetic descriptor carries the element type plus the array field's
                    // read-only state and presentation hints (Min/Max/Step/Widget).
                    ...
                    changed |= DrawValueWidget(element, elementField, "##elemval", hooks);
                    UI::SameLine();
                    if (UI::IconButton(fmt::format("{}##remove", RemoveGlyph)))
                    {
                        removeAt = i;
                    }
                    continue;
                ...
                if (collapsible)
                {
                    ...
                    if (open)
                    {
                        changed |= DrawStructFields(element, elementInfo, hooks);
                    }
                    continue;
                }
    ...
    namespace
    {
        ...
        // Warns once per field descriptor when its resolved widget hint was degraded. The seen-set
        // is keyed on the FieldDescriptor pointer — stable for the registry's lifetime — and read on
        // the single render thread, so it needs no synchronization. It is cleared when the owning
        // registry changes, so a freed descriptor's pointer cannot be reused as a stale key.
    ...
    bool DrawFieldWidget(void* fieldPtr, const FieldDescriptor& field, const InspectorHooks& hooks)
    {
        ...
        // BeginTable inside a cell. When the field resolves Collapsible, the struct instead folds
        // under a full-width header whose body is the same recursion.
        if (field.Class == FieldClass::Struct)
        {
            ...
            if (display.Collapsible.value_or(false))
            {
                ...
                return DrawStructFields(fieldPtr, nested, hooks);
            }
        ...
        // An array draws an Add button on its own row, then each element's fields beneath it. Each
        // element is a foldable header by default so a long list stays scannable; a field resolving
        // Collapsible=false flattens the elements into indented rows instead.
    ...
    namespace
    {
        bool DrawValueWidget(void* fieldPtr, const FieldDescriptor& field, string_view valueLabel,
                             const InspectorHooks& hooks)
        {
            // The Reference field carries its own read-only handling (drop target vs. label); every
            // other class shares one Disabled scope keyed on ReadOnly.
            if (field.Class == FieldClass::Reference)
            {
                bool changed = false;
                if (hooks.DrawReference)
                {
                    changed = hooks.DrawReference(fieldPtr, field, valueLabel);
                }
                else
                {
                    UI::TextDisabled("(reference)");
                }
                if (!field.Tooltip.empty())
                {
                    UI::Tooltip(field.Tooltip);
            ...
            {
                if (hooks.DrawAssetHandle)
                {
                    changed = hooks.DrawAssetHandle(fieldPtr, field, valueLabel);
                }
                else
                {
                    // Bare fallback (no asset-resolving hook): the leading u64 of any AssetHandle is
                    // its id, shown read-only so a game inspector still reads what is referenced.
                    u64 id = 0;
                    std::memcpy(&id, fieldPtr, sizeof(id));
                    UI::TextDisabled(id != 0 ? fmt::format("Asset {:#018x}", id)
                                             : string{"(none)"});
                }
                break;
            }
            ...
            {
                changed = DrawEnum(fieldPtr, field, valueLabel, hooks);
                break;
            }
