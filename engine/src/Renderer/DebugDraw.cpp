...
namespace Veng::Renderer
{
    ...
    {
        m_LineVertices.emplace_back(a, color);
        m_LineVertices.emplace_back(b, color);
    }

    void DebugDraw::DrawBox(const AABB& box, const vec4 color)
    {
                               ...
                               const u32 segments)
    {
        ...
        for (u32 i = 0; i < count; ++i)
        {
            ...
            DrawLine(center + radius * vec3(pa.x, pa.y, 0.0f),
                     center + radius * vec3(pb.x, pb.y, 0.0f), color);
            DrawLine(center + radius * vec3(pa.x, 0.0f, pa.y),
                     center + radius * vec3(pb.x, 0.0f, pb.y), color);
            DrawLine(center + radius * vec3(0.0f, pa.x, pa.y),
                     center + radius * vec3(0.0f, pb.x, pb.y), color);
        }
    }

    void DebugDraw::DrawFrustum(const mat4& invViewProjection, const vec4 color)
    {
    ...
    void DebugDraw::DrawTransform(const mat4& transform, const f32 scale)
    {
        const vec3 origin = vec3(transform[3]);
        DrawLine(origin, origin + scale * vec3(transform[0]), vec4(1.0f, 0.0f, 0.0f, 1.0f));
        DrawLine(origin, origin + scale * vec3(transform[1]), vec4(0.0f, 1.0f, 0.0f, 1.0f));
        DrawLine(origin, origin + scale * vec3(transform[2]), vec4(0.0f, 0.0f, 1.0f, 1.0f));
    }
