...
namespace Veng
{
    ...
    [[nodiscard]] inline GLFWwindow* GetGlfwWindow(const Window& window)
    {
        return window.m_Handle;
    }

    /// @brief Returns the Vulkan surface for a window.
    [[nodiscard]] inline vk::SurfaceKHR GetVkSurface(const Window& window)
    {
        return window.GetNative().Surface;
    }
}
...
namespace Veng::Renderer
{
    ...
    [[nodiscard]] inline vk::Instance GetVkInstance(const Context& context)
    {
        return context.GetNative().Instance;
    }

    /// @brief Returns the Vulkan physical device.
    [[nodiscard]] inline vk::PhysicalDevice GetVkPhysicalDevice(const Context& context)
    {
        return context.GetNative().PhysicalDevice;
    }

    /// @brief Returns the Vulkan logical device.
    [[nodiscard]] inline vk::Device GetVkDevice(const Context& context)
    {
        return context.GetNative().Device;
    }

    /// @brief Returns the graphics queue.
    [[nodiscard]] inline vk::Queue GetVkGraphicsQueue(const Context& context)
    {
        return context.GetNative().GraphicsQueue;
    }

    /// @brief Returns the present queue.
    [[nodiscard]] inline vk::Queue GetVkPresentQueue(const Context& context)
    {
        return context.GetNative().PresentQueue;
    }

    /// @brief Returns the Vulkan surface associated with a context.
    [[nodiscard]] inline vk::SurfaceKHR GetVkSurface(const Context& context)
    {
        return context.GetNative().Surface;
    }

    /// @brief Returns the VMA allocator.
    [[nodiscard]] inline VmaAllocator GetVmaAllocator(const Context& context)
    {
        return context.GetNative().Allocator;
    }

    /// @brief Returns the pipeline cache.
    [[nodiscard]] inline vk::PipelineCache GetVkPipelineCache(const Context& context)
    {
        return context.GetNative().PipelineCache;
    }

    /// @brief Returns the underlying Vulkan buffer handle.
    [[nodiscard]] inline vk::Buffer GetVkBuffer(const Buffer& buffer)
    {
        return buffer.GetNative().Buffer;
    }

    /// @brief Returns the VMA allocation for a buffer.
    [[nodiscard]] inline VmaAllocation GetVmaAllocation(const Buffer& buffer)
    {
        return buffer.GetNative().Allocation;
    }
    ...
    [[nodiscard]] inline vk::Image GetVkImage(const Image& image)
    {
        return image.GetNative().Image;
    }

    /// @brief Returns the VMA allocation for an image.
    [[nodiscard]] inline VmaAllocation GetVmaAllocation(const Image& image)
    {
        return image.GetNative().Allocation;
    }

    /// @brief Returns the VMA allocation info for an image.
    [[nodiscard]] inline const VmaAllocationInfo& GetVmaAllocationInfo(const Image& image)
    {
        return image.GetNative().AllocationInfo;
    }

    /// @brief Returns the underlying Vulkan image view handle.
    [[nodiscard]] inline vk::ImageView GetVkImageView(const ImageView& imageView)
    {
        return imageView.GetNative().ImageView;
    }

    /// @brief Returns the underlying Vulkan sampler handle.
    [[nodiscard]] inline vk::Sampler GetVkSampler(const Sampler& sampler)
    {
        return sampler.GetNative().Sampler;
    }

    /// @brief Returns the underlying Vulkan shader module handle.
    [[nodiscard]] inline vk::ShaderModule GetVkShaderModule(const ShaderModule& shader)
    {
        return shader.GetNative().Module;
    }

    /// @brief Returns the underlying Vulkan descriptor set handle.
    [[nodiscard]] inline vk::DescriptorSet GetVkDescriptorSet(const DescriptorSet& descriptorSet)
    {
        return descriptorSet.GetNative().Set;
    }

    /// @brief Returns the underlying Vulkan descriptor set layout handle.
    [[nodiscard]] inline vk::DescriptorSetLayout
    GetVkDescriptorSetLayout(const DescriptorSetLayout& layout)
    {
        return layout.GetNative().Layout;
    }

    /// @brief Returns the underlying Vulkan pipeline layout handle.
    [[nodiscard]] inline vk::PipelineLayout GetVkPipelineLayout(const PipelineLayout& layout)
    {
        return layout.GetNative().Layout;
    }

    /// @brief Returns the underlying Vulkan pipeline handle for a graphics pipeline.
    [[nodiscard]] inline vk::Pipeline GetVkPipeline(const GraphicsPipeline& pipeline)
    {
        return pipeline.GetNative().Pipeline;
    }

    /// @brief Returns the underlying Vulkan pipeline handle for a compute pipeline.
    [[nodiscard]] inline vk::Pipeline GetVkPipeline(const ComputePipeline& pipeline)
    {
        return pipeline.GetNative().Pipeline;
    }

    /// @brief Returns the underlying Vulkan command buffer handle.
    [[nodiscard]] inline vk::CommandBuffer GetVkCommandBuffer(const CommandBuffer& commandBuffer)
    {
        return commandBuffer.GetNative().CommandBuffer;
    }

    /// @brief Returns the underlying Vulkan fence handle.
    [[nodiscard]] inline vk::Fence GetVkFence(const Fence& fence)
    {
        return fence.GetNative().Fence;
    }

    /// @brief Returns the underlying Vulkan semaphore handle for a binary semaphore.
    [[nodiscard]] inline vk::Semaphore GetVkSemaphore(const Semaphore& semaphore)
    {
        return semaphore.GetNative().Semaphore;
    }
    ...
    [[nodiscard]] inline vk::Semaphore GetVkSemaphore(const TimelineSemaphore& semaphore)
    {
        return semaphore.GetNative().Semaphore;
    }
}
